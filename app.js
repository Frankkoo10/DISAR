    ...
    container.innerHTML = items.map(p => `
        <div onclick="openProductModal(${p.id})" class="drip-card bg-[#121212] border border-white/5 cursor-pointer group rounded-sm overflow-hidden flex flex-col justify-between">
            <div class="relative aspect-[4/5] bg-black overflow-hidden">
                ...
                ${p.is_latest_drop ? '<span class="absolute top-3 left-3 bg-white text-black font-bold text-[9px] uppercase px-2 py-0.5 tracking-widest">NEW DROP</span>' : ''}
            </div>
            <div class="p-4 flex flex-col justify-between flex-grow">
                <div>
                    <span class="text-[10px] text-gray-500 uppercase tracking-widest block font-bold">${p.category}</span>
                    <h4 class="font-['Syne'] font-bold text-base uppercase mt-1 tracking-tight text-white group-hover:text-gray-300 transition line-clamp-1">${p.name}</h4>
                </div>
                <div class="flex justify-between items-center mt-4 pt-3 border-t border-white/5">
                    <span class="font-bold text-sm text-gray-200">$${p.price.toLocaleString('es-AR')}</span>
                    <span class="text-[10px] uppercase tracking-widest underline text-gray-400 group-hover:text-white">Ver Talles</span>
                </div>
            </div>
        </div>
    `).join('');
